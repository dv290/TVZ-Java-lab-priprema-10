...
public class DelivererDatabase {
    ...
    public static Set<Deliverer> getAllDeliverersFromDatabase()
            throws SQLException, IOException
    {
        ...
        try (Connection connection = connectToDatabase();
             ...
             ResultSet resultSet = sqlStatement.executeQuery("SELECT *" + "FROM DELIVERER"))
        {
            while (resultSet.next()) {
                Deliverer newDeliverer = getDeliverersFromResultSet(resultSet);
    ...
    private static Deliverer getDeliverersFromResultSet(ResultSet resultSet)
        throws SQLException
    {
        ...
        return new Deliverer.DelivererBuilder()
    ...
    public static Contract getContractById(Long contractId) {
        ...
        try (Connection connection = connectToDatabase();
            ...
            log.error("SQL / IO exception in DelivererDatabase class! ");
            throw new RuntimeException("Database error occurred", e);
        }
    }

    public static void insertNewDelivererToDatabase(Deliverer deliverer)
            throws SQLException, IOException
    {
        try (Connection connection = connectToDatabase();
             PreparedStatement stmt = connection.prepareStatement(
                     "INSERT INTO DELIVERER (FIRST_NAME, LAST_NAME, SALARY) VALUES(?,?,?)",
                     Statement.RETURN_GENERATED_KEYS))
        {
            ...
            try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    deliverer.setId(generatedKeys.getLong(1));
                }
            }
        }
        finally {activeConnectionWithDatabase = false;}
